...
can make one big file. Defaulting to csv because that is part of our standard
operating procedure. The data file for October does not have key variables in
it, so we ignore october for now.

```{r}
## or.df8 <- read.csv("Data/PEPP UI Monthly Dataset 100116-103116 No Password No Formulas.csv")
## or.df8$fonum <- or.df8$FO_277
...
or.df.TOTAL  <- rbind(or.df1to7,or.df9[,names(or.df1to7)])
```
...
## Check
tmp <- table(months(or.df.TOTAL$claim.date),exclude=c())
